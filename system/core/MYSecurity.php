  ...
  public static function xssClean($str, $isImage = false) {
    ...
    if (stripos($str, '%') !== false) {
      do {
        ...
        $str = preg_replace_callback('#%(?:\s*[0-9a-f]){2,}#i', ['MYSecurity', 'urldecodespaces'], $str);
      } while ($oldStr !== $str);
      unset ($oldStr);
    }

    $str = preg_replace_callback("/[^a-z0-9>]+[a-z0-9]+=([\'\"]).*?\\1/si", ['MYSecurity', 'convertAttribute'], $str);
    $str = preg_replace_callback('/<\w+.*/si', ['MYSecurity', 'decodeEntity'], $str);
    $str = self::removeInvisibleCharacters($str);
    $str = str_replace("\t", ' ', $str);
    ...
    foreach ($words as $word) {
      ...
      $str = preg_replace_callback('#(' . substr($word, 0, -3) . ')(\W)#is', ['MYSecurity', 'compactExplodedWords'], $str);
    }

    do {
      $original = $str;

      preg_match('/<a/i', $str) && $str = preg_replace_callback('#<a(?:rea)?[^a-z0-9>]+([^>]*?)(?:>|$)#si', ['MYSecurity', 'jsLinkRemoval'], $str);

      preg_match('/<img/i', $str) && $str = preg_replace_callback('#<img[^a-z0-9]+([^>]*?)(?:\s?/?>|$)#si', ['MYSecurity', 'jsImgRemoval'], $str);
    ...
    do {
      $oldStr = $str;
      $str = preg_replace_callback($pattern, ['MYSecurity', 'sanitizeNaughtyHtml'], $str);
    } while ($oldStr !== $str);
    unset($oldStr);
